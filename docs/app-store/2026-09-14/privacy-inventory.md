...
| Prompts, project metadata, conversations and jobs | Owner-scoped Firestore records migrated from local storage | Cloud projects/models and prompt suggestions are implemented; the selected image preview and words go to Vertex AI. Suggestions, usage counts and settlement remain private in Firestore; interrupted native requests retain an account-scoped local draft cleared by account deletion. Conversational chat also sends bounded recent history and the saved creative brief to Vertex AI; public-to-owner replies and actual charges persist in Firestore, and private provider state is not client-readable. Concept generation and provider retention review remain |
