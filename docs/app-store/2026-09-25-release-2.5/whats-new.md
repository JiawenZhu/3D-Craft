# Version 2.5 (Build 51) — App Store Release Metadata & What's New

## 1. What's New in This Version (此版本的新增功能)

### English (U.S.) — en-US
```text
3D Craft 2.5 introduces an all-new multi-engine 3D & animation studio with interactive previews, instant local caching, and developer API access:
...
```text
3D Craft 2.5 带来全新多引擎 3D 与角色动画工坊，支持模型实时 3D 预览、本地秒级缓存与开发者 API 联动：
...
## 2. TestFlight "What to Test" Notes (TestFlight 测试说明)

### English (en-US)
```text
3D Craft 2.5 (Build 51) focuses on multi-engine model selection, animation previews, and local GLB caching:
...
```text
3D Craft 2.5 (Build 51) 重点测试多引擎选择、动画预览及本地 GLB 模型缓存：
