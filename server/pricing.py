...
def catalog(today=None):
    ...
    models = {
        ...
        "gemini-3.1-flash-image": row("Gemini 3.1 Flash Image · Nano Banana 2", "image", .067,
            "Per 1K output image; the fast concept option. Input $0.50/M and text/thinking output $3/M tokens are additional.",
            "每张 1K 输出图 $0.067（快速概念图）。另计输入 $0.50/百万、文字及思考输出 $3/百万 tokens。", GOOGLE_SOURCE,
            inputPerMillion=.5, outputPerMillion=3.),
