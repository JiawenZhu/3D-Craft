...
struct CommunityGamesView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                Text(store.t("Games open on their creators’ websites. 3D Craft does not control external content.", "游戏会在创作者的网站中打开，外部内容由创作者提供。"))
        ...
        .sheet(isPresented:$submitting) { CommunitySubmissionView { Task { await load() } } }
        ...
        .alert(store.t("Report game", "举报游戏"),isPresented:Binding(get:{reporting != nil},set:{if !$0 { reporting=nil }})) {
            ...
            Button(store.t("Report & hide", "举报并隐藏")) { if let game=reporting { perform(game) { _=try await store.communityRequest("/games/\(game.id)/report",method:"POST",body:["reason":reportReason]);await load() } } }
    ...
    private func gameCard(_ game: CommunityGame, rank: Int) -> some View {
        VStack(alignment:.leading,spacing:14) {
            ...
            }.buttonStyle(CraftPressStyle()).accessibilityLabel(store.t("Play ", "玩 ")+game.title)
                ...
                Menu {
                    if let url = URL(string: game.link) {
                        ...
                        Button {
                            ...
                            Label(store.t("Open in Safari", "在 Safari 中打开"), systemImage: "safari")
                        }
                    }
                    if game.isMine {
            ...
            if !game.reachable { Text(store.t("Link unavailable — check it again to return to rankings.", "链接暂不可用，重新检查后可返回排行榜。")).font(.caption).foregroundStyle(.orange) }
            HStack(spacing:12) {
                Button { play(game)
                } label: {
                ...
                .disabled(busy.contains(game.id))
            ...
            if busy.contains(game.id) {
                Label(store.t("Saving your vote…", "正在保存你的投票……"), systemImage: "arrow.triangle.2.circlepath")
    ...
    private func play(_ game: CommunityGame) {
        guard let url=URL(string:game.link), url.scheme=="https", url.host != nil else {
            error=store.t("This game link cannot be opened.", "此游戏链接无法打开。"); return
        }
    ...
    private func load(more: Bool = false) async {
        ...
        do {
            if mine {
                ...
                let result=try await store.communityRequest("/games?category=\(category.rawValue)&offset=\(more ? games.count : 0)") as? [String:Any] ?? [:]
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                Section {
                    Text(store.t("Your game title, creator name, description and link will be public. Player votes decide the rankings.", "游戏名称、创作者名称、介绍和链接将公开展示，玩家投票决定排名。")).font(.caption)
                    Button(store.t("Publish game", "发布游戏")) { run {
                        _=try await store.communityRequest("/games",method:"POST",body:["title":title,"creator":creator,"description":description,"url":checkedLink ?? link,"played":played,"rightsConfirmed":rightsConfirmed])
                        onPosted();dismiss()
